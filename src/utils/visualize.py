"""Local accuracy eyeball: extract local PDFs and render annotated page images.
...
Guarded to ENVIRONMENT == "local". Run: `uv run python -m src.visualize`
"""
...
def _annotate(pdf_bytes: bytes, rec: InvoiceRecord, stem: str) -> list[Path]:
    ...
    for fkey, fr in rec.fields.items():
        if fr.value is None or not fr.polygon or not fr.page:
            continue
        by_page.setdefault(fr.page, []).append((fkey, fr.value, fr.confidence, fr.polygon))
    ...
    with fitz.open(stream=pdf_bytes, filetype="pdf") as doc:
        for page_no, fields in sorted(by_page.items()):
            ...
            for fkey, _val, conf, poly in fields:
                ...
                rect = fitz.Rect(
                    min(xs) * _PT_PER_INCH, min(ys) * _PT_PER_INCH,
                    max(xs) * _PT_PER_INCH, max(ys) * _PT_PER_INCH,
                )
...
async def run() -> None:
    ...
    # The Azure/OpenAI SDKs log every HTTP request at INFO — mute to WARNING.
    for noisy in ("azure", "azure.core.pipeline.policies.http_logging_policy", "httpx", "openai"):
        logging.getLogger(noisy).setLevel(logging.WARNING)
    if settings.ENVIRONMENT != "local":
        log.warning("visualize only runs when ENVIRONMENT=local (got %s)", settings.ENVIRONMENT)
        return

    _OUT_DIR.mkdir(exist_ok=True)
    pdfs = sorted(_INVOICE_DIR.glob("*.pdf"))[:_LIMIT]
            ...
            try:
                ...
                for inv in invoices:
                    ...
                    log.info(
                        ...
                        pdf_path.name, inv.index, filled, len(rec.fields),
                        rec.needs_review, len(imgs),
                    )
